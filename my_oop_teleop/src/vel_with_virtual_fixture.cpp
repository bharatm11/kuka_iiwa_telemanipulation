...
int main(int argc, char * argv[])
{
	...
	*/
	...
	ros::Publisher robot_pose_recorder = nh_.advertise<geometry_msgs::Twist>("/iiwa_fk",10);
	...
	if (initialized)
	{
		if(initialized_phantom)
		{
			...
			else
			{
				...
				robot_pose_recorder.publish(xyz);
				if((blocking==0)&&(sqrt(compensated_ft.wrench.force.x*compensated_ft.wrench.force.x+compensated_ft.wrench.force.y*compensated_ft.wrench.force.y+compensated_ft.wrench.force.z*compensated_ft.wrench.force.z)<7))
				{
					msg1.data=6;
					pub1.publish(msg1);
					double z_gain = 1;
					...
					xyz_command.linear.x = pos.pose.position.z*1.8;//xyz_phantom.linear.y;
